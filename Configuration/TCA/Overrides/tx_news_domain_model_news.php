...
foreach ($GLOBALS['TCA']['tx_news_domain_model_news']['types'] as $type => $tmp) {
	...
	if ($replacedTca = preg_replace('/(\s*)related_from(\s*)(;|,)/', 'related_from,related_fsmediaalbums$3', $GLOBALS['TCA']['tx_news_domain_model_news']['types'][$type]['showitem'])) {
...
\TYPO3\CMS\Core\Utility\ArrayUtility::mergeRecursiveWithOverrule($GLOBALS['TCA']['tx_news_domain_model_news']['columns'], $additionalColumns);
