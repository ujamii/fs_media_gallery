...
class JavaScriptViewHelper extends AbstractViewHelper {
	...
	public function initializeArguments() {
		$this->registerArgument('name', 'string', 'If empty, a combination of plugin name and the uid of the cObj is used.');
		$this->registerArgument('moveToFooter', 'boolean', 'If TRUE, adds the script to the document footer by PageRenderer->addJsFooterInlineCode().');
	}
	...
	public function render() {
		...
		if (!empty($this->arguments['moveToFooter']) && TYPO3_MODE === 'FE') {
			...
			$GLOBALS['TSFE']->getPageRenderer()->addJsFooterInlineCode($blockName, $content, $GLOBALS['TSFE']->config['config']['compressJs']);
			return '';
		} else {
			$lb = "\n";
